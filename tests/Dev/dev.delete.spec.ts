...
async function createUser(api: APIRequestContext, name: string, email: string, age: number) {
  return await api.post('/dev/users', {
    data: {
      name: name,
      email: email,
      age: age
...
async function deleteUser(api: APIRequestContext, email: string) {
  return await api.delete(`/dev/users/${email}`);
}

async function deleteUserWithoutEmailHeader(api: APIRequestContext) {
  return await api.delete('/dev/users');
}
...
test.describe('Validate DELETE Endpoint - List of users', () => {
  ...
  test('DELETE - Delete user using non existing email - Status Code 404', async () => {
    ...
    const res = await api.delete(`/dev/users/${name}`);
  ...
  test('DELETE - Delete specific user by age - Status Code 404', async () => {
    ...
    const res = await api.delete(`/dev/users/${age}`);
  ...
  test('DELETE - Delete user without Email Header - Status Code 400', async () => {
    const api = await getRequestContext(true);
