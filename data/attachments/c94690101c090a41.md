...
- Name: Dev/dev.put.spec.ts >> Validate PUT Endpoint - Update a user >> PUT - Update user with age less than 1 - Status 400
- Location: tests/Dev/dev.put.spec.ts:192:7
...
> 21  |   return await api.post('/dev/users', {
  ...
  31  |   return await api.put(`/dev/users/${email}`, {
  32  |     data: {
  33  |       name: name,
  34  |       email: email,
  35  |       age: age
  ...
  41  |   return await api.put(`/dev/users/${email}`, {
  42  |     data: {
  43  |       email: email,
  44  |       age: age
  45  |     }
  ...
  50  |   return await api.put(`/dev/users/${name}`, {
  51  |     data: {
  52  |       name: name,
  53  |       age: age
  54  |     }
  ...
  59  |   return await api.put(`/dev/users/${email}`, {
  60  |     data: {
  61  |       name: name,
  62  |       email: email
  63  |     }
  ...
  68  |   return await api.put('/dev/users', {
  69  |     data: {
  70  |       name: name,
  71  |       email: email,
  72  |       age: age
  ...
  78  |   return await api.delete(`/dev/users/${email}`);
  79  | }
  80  | 
