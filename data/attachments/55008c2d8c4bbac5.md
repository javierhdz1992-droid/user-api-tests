...
- Name: Prod/prod.put.spec.ts >> Validate PUT Endpoint - Update a user >> PUT - Update user without Email Header - Status Code 400
- Location: tests/Prod/prod.put.spec.ts:257:7
...
```ts
  ...
  180 | 
  181 |     const updateRes = await api.put(`/prod/users/${email1}`, {
  182 |     data: {
