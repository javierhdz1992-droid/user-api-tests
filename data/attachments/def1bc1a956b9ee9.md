...
- Name: Prod/prod.post.spec.ts >> Validate POST Endpoint - Create user >> POST - Create user without age - Status 400
- Location: tests/Prod/prod.post.spec.ts:105:7
...
```ts
  ...
  32  |   return await api.post('/prod/users', {
  33  |     data: {
  34  |       email: email,
  35  |       age: age
  36  |     }
  ...
  41  |   return await api.post('/prod/users', {
  42  |     data: {
  43  |       name: name,
  44  |       age: age
  45  |     }
...
> 50  |   return await api.post('/prod/users', {
      |                    ^ TypeError: apiRequestContext.post: Invalid URL
  51  |     data: {
  52  |       name: name,
  53  |       email: email
